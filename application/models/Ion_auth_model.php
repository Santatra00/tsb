...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author    Mathew
	 */
	public function register($identity, $password, $additional_data = [], $groups = [])
	{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select([$this->identity_column, 'email', 'id',' password', 'active', "last_login",' uti_nom','uti_prenom'])
					->where($this->identity_column, $identity)
					->limit(1)
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($password, $user->password, $identity))
			{
				...
				// Mettre ici la photo de l'utilisateur
				$this->set_session($user, $isAdmin, [$user->uti_nom, $user->uti_prenom]);
	 ...
	 * @author jrmadsen67
	 */
	public function set_session($user, $isAdmin="FALSE", $infoPerso=[])
	{
	...
	public function login_remembered_user()
	{
		...
		$query = $this->db
						->select("users.".$this->identity_column . ', users.uti_nom, users.uti_prenom, users.email, users.remember_code, users.id, password, users.active, users.last_login')
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($token->validator, $user->remember_code, $identity))
			{
				...
				$this->set_session($user, $isAdmin, [$user->nom, $user->prenom]);
	...
	public function getInfoUser($id){
		$query = $this->db->select(["users.uti_nom", "users.uti_prenom"])
			->where(array(
				"users.id"=>$id
			))
			->limit(1)
	...
	public function getInfoUserByPersonnel($id){
		$query = $this->db->select(["users.uti_nom", "users.uti_prenom", "users.username", "users.id"])
			->where(array(
				"users.id"=>$id
			))
			->limit(1)
	...
	public function getNoGroup(){
		$query = $this->db->select("*")
			->get('groups');
		return $query->result();
	}
	public function getNoGroupSelected($id){
		$query = $this->db->select("groups.id")
		->join("users_groups", "users_groups.group_id=groups.id", "INNER")
